...
Write-Host "â•'     MCP Server Repository Standards Checker        â•'" -ForegroundColor Magenta
...
if (-not (Test-Path "pyproject.toml") -and -not (Test-Path ".git")) {
    Write-Host "[FAIL] Error: Must run from repository root" -ForegroundColor Red
    exit 1
}
...
if ($fastmcpIssues.Count -eq 0) {
    ...
    Write-Host "  [FAIL] Found $($fastmcpIssues.Count) description= parameters" -ForegroundColor Red
...
if ($mcpbIssues.Count -eq 0) {
    ...
    Write-Host "  [FAIL] Missing $($mcpbIssues.Count) MCPB files" -ForegroundColor Red
...
if ($ciIssues.Count -eq 0) {
    ...
    Write-Host "  [FAIL] Missing $($ciIssues.Count) CI/CD workflows" -ForegroundColor Red
...
if ($structureIssues.Count -eq 0) {
    ...
    Write-Host "  [FAIL] Missing $($structureIssues.Count) required directories" -ForegroundColor Red
...
if ($docsIssues.Count -eq 0) {
    ...
    Write-Host "  [FAIL] Missing $($docsIssues.Count) documentation files" -ForegroundColor Red
...
|----------|-------|--------|
| FastMCP 2.12+ | $($results.Scores.FastMCP)/10 | $(if($results.Scores.FastMCP -ge 8){"[OK]"}else{"[FAIL]"}) |
| MCPB Packaging | $($results.Scores.MCPB)/10 | $(if($results.Scores.MCPB -ge 8){"[OK]"}else{"[FAIL]"}) |
| CI/CD | $($results.Scores.CICD)/10 | $(if($results.Scores.CICD -ge 7){"[OK]"}else{"[FAIL]"}) |
| Test Scaffold | $($results.Scores.Tests)/10 | $(if($results.Scores.Tests -ge 7){"[OK]"}else{"[FAIL]"}) |
| Folder Structure | $($results.Scores.Structure)/10 | $(if($results.Scores.Structure -ge 8){"[OK]"}else{"[FAIL]"}) |
| Documentation | $($results.Scores.Documentation)/10 | $(if($results.Scores.Documentation -ge 8){"[OK]"}else{"[FAIL]"}) |
| Repo Cleanliness | $($results.Scores.Cleanliness)/10 | $(if($results.Scores.Cleanliness -ge 8){"[OK]"}else{"[FAIL]"}) |
| Modern Tooling | $($results.Scores.Tooling)/10 | $(if($results.Scores.Tooling -ge 8){"[OK]"}else{"[FAIL]"}) |

---

## [FAIL] Issues Found ($($results.Issues.Count))
...
Write-Host "`nâ•"â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•-" -ForegroundColor Magenta
Write-Host "â•'               Analysis Complete!                   â•'" -ForegroundColor Magenta
Write-Host "â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•`n" -ForegroundColor Magenta
