...
Write-Host "â•‘         SOTA Script Synchronization (Spoke)         â•‘" -ForegroundColor Cyan
...
if (-not (Test-Path $siblingHubPath)) {
    Write-Host "[FAIL] Error: Could not find mcp-central-docs sibling directory at:" -ForegroundColor Red
...
foreach ($mapping in $sotaMapping) {
    if (-not (Test-Path $mapping.Source)) {
        Write-Host "[WARN]  Warning: SOTA source not found: $($mapping.Source)" -ForegroundColor Yellow
        continue
    }
    ...
    if (Test-Path $mapping.Target) {
        ...
        if ($sourceHash -eq $targetHash) {
            Write-Host "  [skip]  $(Split-Path $mapping.Target -Leaf) is already up-to-date." -ForegroundColor Gray
            $skipped++
        }
        else {
            Copy-Item $mapping.Source $mapping.Target -Force
            Write-Host "  [OK] Updated $(Split-Path $mapping.Target -Leaf) to latest SOTA." -ForegroundColor Green
            $updated++
        }
    }
    else {
