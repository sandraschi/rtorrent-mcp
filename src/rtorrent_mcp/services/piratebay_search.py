...
async def search_piratebay_tv(
    ...
    try:
        ...
        if group:
            ...
            search_term_with_group = f"{query} {group}"
        else:
            search_term_with_group = None
        ...
        async with aiohttp.ClientSession() as session:
                        ...
                        if table:
                            ...
                            for row in rows:
                                try:
                                    ...
                                    title_link = title_cell.select_one(
                                        'a[href*="/torrent/"]'
                                    ) or title_cell.select_one("a[title]")
                                    if not title_link:
                                        continue
                                    ...
                                    uploaded = (
                                        cells[2].text.strip() if len(cells) > 2 else "Unknown"
                                    )
            ...
            if group and results:
                ...
                for r in results:
                    if r["release_group"].upper() == group.upper():
                        ...
                        matches = sum(
                            ...
                            or term.replace(" ", "") in title_lower.replace(" ", "")
                        )
                        if matches >= max(
                            1, len(query_terms) - 1
                        ):  # Allow one mismatch for formatting
                ...
                if preferred_results:
                    ...
                    logger.info(
                        f"Found {len(preferred_results)} {group} results matching query from Pirate Bay"
                    )
                    return preferred_results[:10]
                else:
                    # No preferred group found in group-specific search, try fallback
                    logger.warning(
                        f"No {group} results matching query found, trying broader search"
                    )
                        ...
                        if table:
                            ...
                            for row in rows:
                                try:
                                    ...
                                    score = calculate_tv_quality_score(
                                        title, resolution, group or "MeGusta"
                                    )
                                    release_group = detect_tv_release_group(title)
                                    episode_info = extract_episode_info(title)
...
def register_tv_search_tools(mcp):
    ...
    async def search_tv_series(
        query: str, resolution: str = "1080p", group: str = "MeGusta"
    ) -> list[dict]:
