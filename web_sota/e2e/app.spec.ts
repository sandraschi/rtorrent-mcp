...
test.describe('Frontend', () => {
    test('dashboard loads with stat cards', async ({ page }) => {
        await page.goto('/');
        await expect(page.locator('text=Magnet')).toBeVisible({ timeout: 15000 });
        await expect(page.locator('text=Torrent list')).toBeVisible();
    });

    test('each page loads without crash', async ({ page }) => {
        const pages = ['/', '/status', '/tools', '/apps', '/chat', '/help', '/settings'];
...
test.describe('REST API', () => {
    ...
    test('POST /api/rtorrent/magnet with invalid body returns 400', async ({ request }) => {
        const resp = await request.post('/api/rtorrent/magnet', { data: {} });
        expect(resp.status()).toBe(400);
    });

    test('GET /api/rtorrent/torrents returns 503 when rTorrent is unreachable', async ({ request }) => {
        const resp = await request.get('/api/rtorrent/torrents');
        // 401 (no API key) or 503 (rTorrent down) — both indicate the route exists and responds
        expect([401, 503]).toContain(resp.status());
    });

    test('GET /api/rtorrent/status returns connected or error', async ({ request }) => {
        const resp = await request.get('/api/rtorrent/status');
        const body = await resp.json();
