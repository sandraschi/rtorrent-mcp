...
import { Activity, Link2, Server, ListTree } from "lucide-react";
...
export function Dashboard() {
    ...
    async function submitMagnet(e: FormEvent) {
        ...
        setMagnetBusy(true);
        try {
            const r = await fetch("/api/rtorrent/magnet", {
                method: "POST",
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-400">
                        Live data from the Python bridge: <code className="text-xs text-emerald-300/90">/api/*</code> on
                        the same host as MCP (see <code className="text-xs">web_sota/start.ps1</code>).
                    </p>
                </div>
                <button
                    type="button"
            ...
            {error ? (
                <div className="rounded-lg border border-rose-500/40 bg-rose-950/40 px-4 py-3 text-sm text-rose-100">
                    Bridge unreachable: {error}. Start backend (port 10910) and ensure Vite proxies <code>/api</code>.
                </div>
            ) : null}
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-slate-200">API</CardTitle>
                ...
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-slate-200">rTorrent RPC</CardTitle>
                        <Activity className={`h-4 w-4 ${connected ? "text-emerald-500" : "text-rose-400"}`} />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold text-white">{loading ? "…" : connected ? "Up" : "Down"}</div>
                ...
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-slate-200">Torrents</CardTitle>
                ...
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-slate-200">MCP</CardTitle>
                        <Link2 className="h-4 w-4 text-purple-400" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-bold text-white">/mcp</div>
                        <p className="text-xs text-slate-500">Agents use streamable HTTP here</p>
                    </CardContent>
                </Card>
            </div>
                ...
                <CardContent>
                    <form onSubmit={submitMagnet} className="space-y-3">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {magnetMsg ? <span className="text-sm text-slate-400">{magnetMsg}</span> : null}
                        </div>
                    </form>
                </CardContent>
            </Card>
