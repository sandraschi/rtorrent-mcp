    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-white">Status</h2>
                    <p className="text-slate-400">
                        rTorrent XML-RPC via the same process as MCP. Throttle / upload totals are not exposed here yet;
                        this page lists <strong className="text-slate-200">live torrent rows</strong> from{" "}
            ...
            <div className="grid gap-4 md:grid-cols-3">
                <Card className="border-slate-800 bg-slate-950/50">
                    ...
                    <CardContent>
                        <div className="text-xl font-bold text-white">{loading ? "…" : health?.ok ? "healthy" : "—"}</div>
                        <p className="text-xs text-slate-500">rtorrent-mcp {health?.version ?? ""}</p>
                    </CardContent>
                </Card>
                <Card className="border-slate-800 bg-slate-950/50">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-slate-200">rTorrent</CardTitle>
                        <Link2 className={`h-4 w-4 ${connected ? "text-emerald-500" : "text-rose-400"}`} />
                    </CardHeader>
                    <CardContent>
                        <div className="text-xl font-bold text-white">{loading ? "…" : connected ? "connected" : "offline"}</div>
                ...
                <Card className="border-slate-800 bg-slate-950/50">
                    ...
                    <CardContent>
                        <div className="text-xl font-bold text-white">{loading ? "…" : list.length}</div>
                            ...
                            <tbody>
                                {list.map((t) => (
                                    <tr key={t.hash} className="border-t border-slate-800/80">
                                        ...
                                        <td className="p-3 font-mono text-[10px] text-slate-500 break-all">{t.hash}</td>
                                    </tr>
                                ))}
