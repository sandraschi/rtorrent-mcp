...
const PERSONALITIES: Record<string, string> = {
  "Download Manager": "You are an expert torrent download manager. Help users find, add, and organize torrents. Advise on tracker settings, download paths, and RSS feeds for automated downloading.",
  "Seeder": "You are a seeding specialist focused on ratio management and long-term torrent health. Advise on seeding goals, ratio groups, and disk space management for efficient sharing.",
  "Quick Summarizer": "You are a concise assistant. Answer in 1-3 sentences. Be direct and to the point.",
  "Custom": "",
};

const EXAMPLE_PROMPTS = [
  { group: "Torrents", items: ["Add a new torrent from magnet link", "List all active downloads with ETA", "Prioritize a specific torrent in the queue"] },
  { group: "Queue", items: ["Set maximum active downloads to 5", "Configure schedule for overnight downloading", "Pause all torrents and resume in the morning"] },
  { group: "Settings", items: ["Change default download directory", "Set up RSS feed auto-downloading", "Configure speed limits for peak hours"] },
];
...
export function Chat() {
  const [messages, setMessages] = useState<Message[]>(() => {
    ...
    return [{
      role: "assistant",
      content: "I'm your torrent management assistant. I can help with downloads, queuing, seeding, and qBittorrent configuration. How can I help?",
    ...
    <div className="flex flex-col space-y-4 animate-in fade-in duration-500 h-full" data-testid="chat-page">
      <div className="flex items-center justify-between shrink-0" data-testid="chat-controls">
        <div className="flex items-center gap-4">
          ...
          <span className="text-xs text-muted-foreground bg-muted/30 px-2 py-0.5 rounded font-mono">skill:torrent-manager</span>
        </div>
        <div className="flex items-center gap-2">
