...
async def test_system_tools():
    ...
    try:
        ...
        content = result.content[0].text if result.content else "{}"
        parsed = json.loads(content)
        print(f"✅ System status tool works")
        ...
        print(f"❌ System status tool failed: {e}")
    ...
    try:
        ...
        content = result.content[0].text if result.content else "{}"
        parsed = json.loads(content)
        print(f"✅ Repository analysis tool works")
        ...
        print(f"❌ Repository analysis tool failed: {e}")
...
async def test_torrent_tools():
    ...
    try:
        ...
        content = result.content[0].text if result.content else "{}"
        parsed = json.loads(content)
        print(f"✅ Status tool works")
        print(f"   Status: {parsed.get('status', 'unknown')}")
        if 'host' in parsed:
            print(f"   Host: {parsed.get('host')}:{parsed.get('port', 'N/A')}")
    except Exception as e:
        print(f"❌ Status tool failed: {e}")
        print(f"   (This is expected if rTorrent is not running)")
    ...
    try:
        ...
        content = result.content[0].text if result.content else "[]"
        parsed = json.loads(content)
        print(f"✅ List torrents tool works")
        ...
        print(f"❌ List torrents tool failed: {e}")
        print(f"   (This is expected if rTorrent is not running)")
...
async def test_search_tools():
    ...
    try:
        ...
        content = result.content[0].text if result.content else "[]"
        parsed = json.loads(content)
        print(f"✅ Search anime tool works")
        if isinstance(parsed, list):
        ...
        print(f"❌ Search anime tool failed: {e}")
...
async def main():
    ...
    print("="*60)
    print(f"Testing qBTMCP server tools...")
